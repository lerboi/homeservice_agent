...
|-------|--------|-------------------------------|
| `auth-database-multitenancy` | Supabase Auth, 3 client types, middleware guards, RLS policies, **all 62 DB migrations with every table definition**, getTenantId pattern, tenant isolation | ...understand the DB schema, add a migration, debug RLS, modify auth middleware, add a new table, or find any Supabase table definition |
