# Settings — Voco Dashboard

## What this section does
The Settings area covers all configuration for your AI receptionist and business operations. Configure the services you offer and their prices, set your weekly working hours, define service zones with travel buffers, manage notifications and escalation contacts, and customize your AI voice agent behavior.

## Key features
- **Services & Pricing** — Add, edit, and reorder the services your AI receptionist offers callers
- **Working Hours** — Set your available days and time slots for bookings
- **Service Zones & Travel** — Define geographic coverage areas and travel buffer times between jobs
- **Notifications & Escalation** — Configure SMS/email alerts and add emergency contacts for urgent calls
- **AI & Voice Settings** — Set your business phone number, choose AI tone, and make a test call

## How to navigate here
- [Services & Pricing](/dashboard/more/services-pricing)
- [Working Hours](/dashboard/more/working-hours)
- [Service Zones](/dashboard/more/service-zones)
- [Notifications](/dashboard/more/notifications)
- [AI Voice Settings](/dashboard/more/ai-voice-settings)

## Common tasks

### How do I add a new service?
Go to [Services & Pricing](/dashboard/more/services-pricing). Click "Add Service" and enter the service name, description, and price range. Drag to reorder services in the list. The AI will mention your services when callers ask.

### How do I set my available hours?
Go to [Working Hours](/dashboard/more/working-hours). Toggle each day on or off and set start and end times. The AI will only offer appointment slots within these hours.

### How do I add an emergency escalation contact?
Go to [Notifications](/dashboard/more/notifications). In the Escalation Contacts section, add the name and phone number. This contact receives an immediate SMS when the AI classifies a call as an emergency.

### How do I make a test call to my AI receptionist?
Go to [AI Voice Settings](/dashboard/more/ai-voice-settings). Click "Make Test Call" — your business number will call you so you can hear how the AI sounds to your callers.

### How do I change the AI's tone or greeting?
Go to [AI Voice Settings](/dashboard/more/ai-voice-settings). Adjust the tone setting (Professional, Friendly, Formal) and customize the greeting script. Click Save to apply changes.

## Related sections
- [Integrations](/dashboard/more/integrations)
- [Billing](/dashboard/more/billing)
