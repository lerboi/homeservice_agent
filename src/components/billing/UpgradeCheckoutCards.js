...
export default function UpgradeCheckoutCards({ previousPlanId }) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {tiers.map((tier, index) => (
            ...
            >
              ...
              {/* Previous plan badge */}
              {previousPlanId === tier.id && (
                <span className="text-xs text-[#475569] bg-stone-100 px-2 py-0.5 rounded-full w-fit mt-1">
                  Your previous plan
                </span>
              )}
              ...
              <button
                ...
                aria-label={`Choose the ${tier.name} plan at $${tier.monthlyPrice} per month`}
                className={`
