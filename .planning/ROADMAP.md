...
**Requirements:** D-01 (Fix 1+2 ship together), D-02 (four-precondition trigger: street + postal + day + urgency), D-03 (new SLOT TIMES section mirrors 61.1 address-validation shape), D-04 (OUTCOME WORDS one-line cross-ref), D-05/D-06 (transcript pull — research-time evidence), D-07 (cross-section interaction trace — research-time), D-08 (OUTCOME WORDS shadowing analysis — research verdict: ship Fix 2 as new section, do NOT amend OUTCOME WORDS body), D-09 (pre-fix UAT — Plan 01), D-10 (prompt invariant tests, EN+ES — Plan 03), D-11 (behavioral E2E test — DEFERRED per RESEARCH.md recommendation), D-12 (live UAT mandatory gate — Plan 04), D-13 (staged rollout — DROPPED per CONTEXT, no infra), D-14 (narrow core: ONE prompt change), D-15 (production-call aggregator SQL — Plan 01), D-16 (SKILL.md closeout — Plan 05).
**Depends on:** Phase 61, Phase 61.1
**Plans:** 6/7 plans executed

Plans:
- [x] 61.2-01-PLAN.md — Wave 0: D-09 pre-fix LiveKit SIP UAT call (manual gate) + D-15 aggregator SQL (`.planning/queries/phase-61-call-aggregator.sql` with Q1/Q2/Q3 named queries) (Wave 0, autonomous: false)
- [x] 61.2-02-PLAN.md — Wave 1: livekit-agent prompt edits — new `_build_slot_times_section(locale)` (Fix 2, EN+ES verbatim) + composition wiring at `prompt.py:1355-1356` + URGENCY four-precondition trigger paragraph in `_build_info_gathering_section` (Fix 1, EN+ES) + one-line SLOT TIMES cross-reference in `_build_outcome_words_section` (D-04, EN+ES) (Wave 1, autonomous: true)
- [x] 61.2-03-PLAN.md — Wave 2: livekit-agent invariant tests — new `tests/test_prompt_slot_times_rule.py` (12 tests covering EN/ES headers, core prohibition, incident-anchored failure-mode example, tool-name presence, silence-license absence, top-attention zone position) + 2 new tests in `tests/test_prompt_info_gathering.py` (Fix 1 trigger phrasing EN+ES) + 1 new test in `tests/test_prompt_outcome_words.py` (D-04 cross-ref EN+ES) (Wave 2, autonomous: true; D-11 behavioral E2E explicitly deferred)
- [x] 61.2-04-PLAN.md — Wave 3: D-12 MANDATORY GATE — live LiveKit SIP UAT call against test tenant verifying all three criteria PASS (tools invoked when info gathered; no slot fabrication including as confirmation question; full booking or clean lead-capture); evidence captured in `61.2-UAT.md` (Wave 3, autonomous: false)
- [x] 61.2-05-PLAN.md — Wave 4: closeout — append Phase 61.2 subsection to `voice-call-architecture/SKILL.md` documenting Fix 1 + Fix 2 + D-04 + new test invariants + lesson (slot fabrication as a confirmation question is a distinct failure class; cross-references existing 61.1 silence-deadlock memory) (Wave 4, autonomous: true)
...
- [x] 61.1-04-PLAN.md — Phase 61.1 closeout: skill sync for prompt reframe + WR-01/02 fixes + new feedback memory capturing the silence-deadlock pattern (Wave 3)
 (completed 2026-05-05)
...
- [x] 59-02-PLAN.md — Wave 1: migration 059 (create customers/jobs/inquiries/customer_calls/job_calls + RLS + Realtime + backfill from leads; keep legacy) + [BLOCKING] schema push [D-01, D-02, D-05, D-06, D-07, D-11, D-12, D-13, D-15, D-16]
...
- [x] 59-06-PLAN.md — Wave 3: Jobs tab rewrite (query /api/jobs, Realtime on jobs table) + new Inquiries tab + JobStatusPills/InquiryStatusPills + sidebar/BottomTabBar nav + chatbot corpus split (customers.md, jobs.md, inquiries.md) [D-08, D-09, D-15]
- [x] 59-07-PLAN.md — Wave 3: Customer detail page (sticky header + Activity/Jobs/Invoices tabs) + CustomerEditModal + CustomerMergeDialog + UnmergeBanner + InquiryFlyout + JobFlyout + human-verify checkpoint [D-10, D-17, D-18, D-19]
- [x] 59-08-PLAN.md — Wave 4: migration 061 drop legacy leads/lead_calls + activity_log.customer_id NOT NULL + [BLOCKING] push with coverage survey + delete /api/leads + delete Lead* components + 4 skill files sync (auth-database-multitenancy, dashboard-crm-system, voice-call-architecture, payment-architecture) [D-01, D-03, D-11, D-12, D-14]
...
> **Status: REVERTED 2026-04-25** (livekit-agent commit `d4a1ee1 revert: undo Phase 64 — restore Phase 63.1 Realtime model state`). Phase 64 shipped the pipeline to production main and exposed regressions worse than the Phase 63.1 issues it was meant to solve. See `Outcome` below. The full per-plan SUMMARY files are retained in `.planning/phases/64-livekit-pipeline-agent-migration/` as historical record. Plans 04/05/06 were never completed — execution aborted mid-Plan-04 UAT.
>
> **Post-mortem — why the swap failed in production:**
> 1. **End-of-turn latency ~7s** (vs the ~500ms penalty estimated pre-phase). GeminiTTS first-byte alone was ~1.3s; LLM thinking + generate was ~4s; VAD commit + endpointing added ~1.6s. Callers consistently hung up mid-response.
> 2. **Self-echo triggered Silero VAD false-interrupts** — BVCTelephony noise cancellation did not fully filter the return audio path over SIP. Greeting was repeatedly cut off after ~3.5s of an ~8s expected playback. `agent_false_interruption` events fired on nearly every call.
> 3. **D-05 model-name typo (`gemini-3.1-flash` → 404 on v1beta)** only surfaced during UAT; the correct identifier is `gemini-3-flash-preview`. Fix was applied mid-UAT (commit `b2254e3`) but did not rescue the larger latency / echo issues.
> 4. **D-03b VAD `min_silence_duration=2.5s` was misported** from Phase 63.1-11's Realtime `silence_duration_ms` parameter, producing further latency. Lowering to `0.55s` helped but not enough.
> 5. **Phase 63.1's known Realtime issues** (`generate_reply` silent-drop, 63.1-07 mute-during-greeting workaround) turned out to be **less user-hostile** than pipeline's latency + echo profile for real callers.
>
> **Outcome:** livekit-agent `main` rolled back to `61a2e6e fix(63.1-11)` via file-level revert commit. All Phase 64 commit history preserved above the revert for the post-mortem record. Phase 64 branch `phase-64-pipeline-migration` deleted from origin. Production runs the Phase 63.1 Realtime stack again. No Phase 64.1 planned — going back to the pipeline would require a different TTS provider (sub-500ms first-byte like Cartesia/Deepgram Aura) AND a stronger echo canceller than BVCTelephony, neither of which are scoped.

**Goal (original — kept for historical context):** Migrate the LiveKit voice agent from the Gemini 3.1 Flash Live audio-to-audio Realtime architecture to a classical pipeline-agent stack (STT + LLM + TTS as discrete plugins). Preserve ALL existing behavior, prompts, tools, tenant logic, post-call pipeline, integrations, and Phase 59/60/63 functionality — this is a pure architectural swap. Motivation: Phase 63.1 live UAT (Plans 01-11) proved the Realtime stack at `livekit-agents==1.5.6` + `livekit-plugins-google==1.5.6` carries 5 upstream unfixed bugs that compound into unreliable call flow — mid-word truncation (`_SegmentSynchronizerImpl` race, #4486/#5096), capability-gated `generate_reply`/`say`/`update_chat_ctx` on 3.1 models (realtime_api.py:707, `mutable_chat_context=False`, `supports_say=False`), and tool-call cancellation on caller VAD (#4441). The LiveKit maintainer has closed workaround PRs (#5251, #5262) pending DeepMind guidance (#5234 still open). No local patching stabilized end-to-end booking. The pipeline architecture uses mature plugin APIs with no capability gates; each component (STT/LLM/TTS) can fail/retry independently. Tradeoff: ~500ms added per-turn latency (audio-to-audio ~300ms → pipeline ~800-1200ms) in exchange for definitively reliable behavior. **UAT revealed the real penalty was ~6s not ~500ms — thesis falsified.**
...
Plans:
- [x] 64-01-PLAN.md — Wave 0 RED: tests/test_pipeline_session.py (12 session-construction invariants — `languages=` plural kwarg guard, `detect_language=False`, silero VAD 2.5s, no turn_detector, all 7 tools, no 63.1-07 mute) + RED updates to test_prompt_greeting_directive.py (D-03c re-frame) + test_prompt_booking.py (D-03d one-liner + preserved check_availability-before-book_appointment invariant)
- [x] 64-02-PLAN.md — Session assembly swap in src/agent.py: add `_build_pipeline_plugins(locale, voice_name)` returning (google.STT(chirp_3, languages=BCP-47, detect_language=False), google.LLM(gemini-3.1-flash, thinking_config=low), GeminiTTS(Zephyr), silero.VAD.load(min_silence_duration=2.5)); replace `google.realtime.RealtimeModel(...)` + `realtime_input_config` + `AgentSession(llm=, tts=)` with pipeline `AgentSession(stt=, llm=, tts=, vad=)`; delete 63.1-07 input-mute scaffolding (`session.input.set_audio_enabled(False)`, `_unmute_after_greeting`, 10s asyncio timer). Turns 12 Plan 01 RED tests GREEN.
- [x] 64-03-PLAN.md — Prompt refactor in src/prompt.py: D-03c re-frame `_build_greeting_section` from "GREETING ALREADY PLAYED — DO NOT REPEAT" / "SALUDO YA REALIZADO — NO SE REPITA" workaround-shaped to "GREETING ALREADY DELIVERED" / "SALUDO YA ENTREGADO" architectural-fact shape (EN+ES, USTED); D-03d compress NO DOUBLE-BOOKING block from 8 lines to a one-liner in both locales (EN+ES, USTED); preserves all Phase 60.3-11 invariants (readback, postal_label, check_availability-before-book_appointment). Turns Plan 01 prompt RED tests GREEN.
- [ ] 64-04-PLAN.md — Push `phase-64-pipeline-migration` branch to origin; Railway preview deploy gate; verify worker registered + no ImportError/TypeError; verify `GOOGLE_APPLICATION_CREDENTIALS` has Cloud Speech v2 scope; capture preview URL for Plan 05 (checkpoint).
- [ ] 64-05-PLAN.md — Live EN UAT call against Railway preview: greeting plays first, agent responds without re-greeting, end-to-end booking flow, calendar correctness at tenant local time, Railway log grep for negative signals (`_SegmentSynchronizerImpl` = 0, `server cancelled tool calls` = 0), capture `[goodbye_race]` v2 payload (text_done/audio_done now False on pipeline), measure per-turn latency, optional ES UAT. Write 64-UAT.md with verdict (PASS/PARTIAL/FAIL) (checkpoint).
- [ ] 64-06-PLAN.md — Merge `phase-64-pipeline-migration` to livekit-agent/main with `--no-ff` (preserves per-plan commit history for clean revert); Railway production redeploy gate; rewrite `.claude/skills/voice-call-architecture/SKILL.md` v2.0 (pipeline is new SOT, Realtime → historical appendix preserving Phase 60.2/60.3/63/63.1 prose); write 64-SUMMARY.md (D-01..D-11 coverage table, per-plan commit table, UAT outcome, lessons, 64.1 candidates, rollback story); update STATE.md `[v6.0 P64]` decision entry; update ROADMAP.md Phase 64 marked 6/6 complete + Phase 60.4 unblocked.
