...
- [ ] 61.2-03-PLAN.md — Wave 2: livekit-agent invariant tests — new `tests/test_prompt_slot_times_rule.py` (12 tests covering EN/ES headers, core prohibition, incident-anchored failure-mode example, tool-name presence, silence-license absence, top-attention zone position) + 2 new tests in `tests/test_prompt_info_gathering.py` (Fix 1 trigger phrasing EN+ES) + 1 new test in `tests/test_prompt_outcome_words.py` (D-04 cross-ref EN+ES) (Wave 2, autonomous: true; D-11 behavioral E2E explicitly deferred)
