...
### Migration & cutover
- **D-01:** Big-bang cutover. One migration creates `customers`, `jobs`, `inquiries`; backfills from existing `leads`/`appointments`/`lead_calls`; drops `leads` and `lead_calls`. Safe because v6.0 is still dev (no prod users).
...
- **D-03:** API routes split: `/api/leads/*` → `/api/customers/*` + `/api/jobs/*` + `/api/inquiries/*`. All dashboard fetch sites and chatbot tools updated in this phase. No deprecation window — clean break.
- **D-04:** Voice agent (Python LiveKit, separate Railway repo) updated in same phase as the DB cutover. Lockstep deploy required (Next.js + agent). Capture deploy ordering in PLAN risk section.

### Entity definitions
- **D-05:** Customer dedup: `UNIQUE(tenant_id, phone_e164)`. Phone normalized to E.164 before insert (existing util in `src/lib/phone.js` if present, else add). Phone is the dedup key and is **immutable** post-create — to "change" a customer's phone, owner uses Merge.
...
- **D-07:** Separate `inquiries` table for unbooked calls. `inquiries.appointment_id` does not exist. `inquiries.status` enum: `open`, `converted`, `lost`. Inquiries that become Jobs (same-call conversion) have `status='converted'` + `converted_to_job_id` audit FK.
...
- **D-10:** Inquiry → Job conversion rule: **same-call auto-convert + manual button for offline follow-ups**.
  ...
  - Different call: caller phones back about a different problem and books → new Job, old inquiry untouched (still `Open`).
...
- **D-19:** Merge UI scope: **per-customer "Merge into another" button + 7-day undo**.
  ...
  - Soft-delete source: source.`merged_into = target_id` + `merged_at` timestamp; row hidden from queries. Repointing done via `UPDATE` on jobs/inquiries/invoices/activity_log/customer_calls/job_calls.
  - Undo: within 7 days, "Undo merge" action restores source (clears merged_into) and reverse-repoints.
...
- `.planning/ROADMAP.md` §"Phase 59: Customer/Job model separation" (line 812) — phase title and dependency
- `.planning/STATE.md` — current milestone (v6.0 in flight); Phase 59 is post-v6.0
...
- `supabase/migrations/` — full migration history; new migration will be 053+ (after 052_integrations_schema)
...
- `.claude/skills/auth-database-multitenancy/SKILL.md` — add `customers`, `jobs`, `inquiries`, `customer_calls`, `job_calls` table definitions; document `record_call_outcome` RPC; document RLS policies; document new Realtime publications; remove `leads` / `lead_calls` from current-table list (note migration in changelog).
- `.claude/skills/dashboard-crm-system/SKILL.md` — Jobs vs Inquiries separation; Customer detail page; merge flow; status pill sets per tab; conversion semantics.
...
- **Existing E.164 normalization** for phone numbers in onboarding/voice flows (locate during research; if missing, add `src/lib/phone/normalize.js`).
...
- **Forward-only migrations**, idempotent SQL, named like `NNN_description.sql` — next number is 053.
...
- **Migration audit log** (track every customer merge in `merge_audit` table) — not needed in dev-phase; revisit when real users arrive.
