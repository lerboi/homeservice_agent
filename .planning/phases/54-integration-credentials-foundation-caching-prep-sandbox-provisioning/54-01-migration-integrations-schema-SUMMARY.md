...
provides:
  - Migration 051 tightening `accounting_credentials.provider` CHECK to `('xero', 'jobber')`
...
## Accomplishments
- Migration file `supabase/migrations/051_integrations_schema.sql` encoding the full D-11 sequence (DELETE → DROP CHECK → ADD CHECK → ADD scopes → ADD last_context_fetch_at) wrapped in BEGIN/COMMIT
...
- `supabase/migrations/051_integrations_schema.sql` — Transactional migration: DELETE QB/FB rows → DROP provider CHECK → ADD new CHECK('xero','jobber') → ADD scopes TEXT[] → ADD last_context_fetch_at TIMESTAMPTZ
...
- **Files modified:** `supabase/migrations/051_integrations_schema.sql`
- **Commit:** `861d417`
