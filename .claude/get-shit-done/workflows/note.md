...
- **Global scope**: `F:/homeservice-agent/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/homeservice-agent/.claude/notes/`)
...
2. Glob `F:/homeservice-agent/.claude/notes/*.md` (if directory exists) — global notes
...
Global (F:/homeservice-agent/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `F:/homeservice-agent/.claude/notes/` — works in any directory
