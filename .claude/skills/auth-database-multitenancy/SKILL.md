...
> ⚠️ **Pending manual application:** migrations `068`–`076` await manual application to the live DB (apply in order: 068, 069, 070, 071, 072, 073, 074, 075, 076). **Apply 075 BEFORE deploying the Next.js app** — the JS `available-slots` + `working-hours` routes name `travel_buffer_mins` explicitly and will error if the column is absent. **Apply 076 (`owner_notification_failures`, LK-B2) BEFORE deploying the main-repo `/api/cron/retry-owner-notifications` cron** (the cron names the table); the Python agent's outbox WRITE is best-effort/fail-open, so the agent is safe to deploy before 076 (it just loses retry durability until then — the in-band send is still attempted). (The Python agent is pre-migration-safe — every tenant read uses `select("*")` + `tenant.get("travel_buffer_mins", 30)`.) Migration `067`'s OpenAI voice CHECK on `tenants.ai_voice` is **superseded by `070_ai_voice_labels.sql`** — the CHECK now allows NULL or the 3 stable labels (`professional`, `friendly`, `local_expert`); the agent maps labels → ElevenLabs voice ids.

**Last updated**: 2026-06-21 (**Test-call verification** — migration `077_test_call_verification.sql` (**PENDING manual apply**, additive + idempotent): adds `tenants.test_call_status text NOT NULL DEFAULT 'none'` (CHECK `none|calling|connected|failed`) + `tenants.test_call_last_at timestamptz` (the `test_call_completed` boolean already exists from migration 002). The new LiveKit `participant_joined` webhook `/api/webhooks/livekit` sets `test_call_completed=true` + `test_call_status='connected'` + `test_call_last_at` when the owner's SIP leg joins a `test-call-*` room (genuine connect); `/api/onboarding/test-call` sets `test_call_status='calling'`. Gates the setup checklist's `make_test_call` item. **LK-B2 owner-notification outbox** — migration `076_owner_notification_outbox.sql`, **PENDING manual apply** (extends the 068–075 tail). New service-role-only table `owner_notification_failures` (durable outbox for failed owner missed-job SMS/email alerts) mirroring `stripe_meter_failures` (071): columns `id uuid PK`, `tenant_id uuid FK→tenants ON DELETE CASCADE`, `notification_key text UNIQUE` (`'{call_id}:{channel}'` — SMS+email retry independently), `channel text CHECK (IN 'sms'|'email')`, `recipient text`, `payload jsonb` (sms `{from_number, body}` / email `{subject, html}`), `failure_reason text`, `attempts int DEFAULT 0`, `last_attempt_at`, `created_at DEFAULT now()`; RLS ENABLED + `REVOKE ALL FROM anon, authenticated`, NO policies (service-role bypasses); index on `created_at`. Written by the Python agent's post-call pipeline (best-effort/fail-open on 3s send timeout/failure), drained by main-repo cron `/api/cron/retry-owner-notifications` (every 5 min — re-send stored payload, delete on success, Sentry@10). Apply BEFORE deploying that cron. Highest migration is now **077** (77 distinct versions, 78 files on disk — duplicate version 030 still exists). No RLS pattern change vs 071. **Prior — 2026-06-20:** M16 P1 "Service-Area gate" — migration `074_service_area_gate.sql` (Capability A): additive, no table rewrites, **PENDING manual apply** alongside 072/073 (and 068–071). Adds `service_zones.cities text[] NOT NULL DEFAULT '{}'` (town/city half of the Service-Area coverage list; `postal_codes[]` is the ZIP half), `tenants.out_of_area_action text NOT NULL DEFAULT 'callback' CHECK (IN 'callback'|'decline_referral'|'trip_fee')` + nullable `tenants.out_of_area_referral_note`, and `inquiries.out_of_area boolean NOT NULL DEFAULT false` (durable flag set by the agent's capture_lead tool via a follow-up service-role UPDATE — the hardened `record_call_outcome` RPC from 069 is deliberately untouched, no RPC overload added). No RLS policy changes. **Prior wave — 2026-06-12:** audit wave 1, migrations 069–071 — all PENDING manual application along with 068. **069 `069_rpc_and_token_lockdown.sql`**: catalog-driven DO block REVOKEs anon/authenticated EXECUTE on ALL SECURITY DEFINER functions (they were all anon-callable — earlier migrations 027/033/062/065 only revoked from PUBLIC, which doesn't touch Supabase's explicit per-role grants; verified live) + pins `search_path = public, pg_temp` on them and on the advisor-flagged SECURITY INVOKER functions; also revokes `increment_calls_used` from client roles, and REVOKEs ALL on `accounting_credentials` + `calendar_credentials` from anon/authenticated (plaintext OAuth tokens were owner-browser-readable via PostgREST). **070 `070_ai_voice_labels.sql`**: `tenants.ai_voice` CHECK now the 3 labels `professional`/`friendly`/`local_expert` (replaces 067's OpenAI names; the agent maps labels → ElevenLabs voice ids). **071 `071_meter_event_outbox.sql`**: new service-role-only `stripe_meter_failures` outbox table + `activity_event_type` enum gains `integration_fetch` and `integration_fetch_fanout` (now 18 values; previously every integration-telemetry INSERT failed the enum cast silently — 0 rows ever wrote). Highest migration is now 075 (75 migrations, 76 files on disk — duplicate version 030 still exists); 072/073/074/075 exist on disk and are PENDING manual apply alongside 068–071. **075 `075_travel_buffer.sql` (M16 P2 owner-adjustable travel buffer):** adds `tenants.travel_buffer_mins int NOT NULL DEFAULT 30` — the slot calculator's default buffer is now this owner-set value (threaded through `calculate_available_slots`/`calculateAvailableSlots` like `slot_duration_mins`) and is enforced forward + backward; default 30 = zero regression; 0 disables buffering; no RLS change. Apply 075 BEFORE deploying the Next.js app (the JS available-slots/working-hours routes name the column; the Python agent reads tenants via `select("*")` and is pre-migration-safe).)
...
| **Migrations** | `supabase/migrations/` | Sequential migrations building full schema — highest is 076 (76 distinct versions, 77 files on disk — duplicate version 030); 072/073/074/075/076 PENDING manual apply |
...
| `supabase/migrations/072_calendar_blocks_provider.sql` | 2026-06-20 quick-win (M15), PENDING application — adds nullable `calendar_blocks.external_event_provider text` so PATCH/DELETE cleanup resolves the OWNING provider (recorded at push time) instead of `is_primary`, fixing wrong-provider cleanup after a primary-calendar switch. Additive metadata-only; legacy NULL rows fall back to `is_primary`. Deploy the calendar_blocks route changes only AFTER this is applied. |
| `supabase/migrations/073_book_appointment_atomic_slot_taken.sql` | 2026-06-20 quick-win (LOW-26), PENDING application — `CREATE OR REPLACE book_appointment_atomic` (signature/ACL/search_path unchanged) wrapping the INSERT in `EXCEPTION WHEN exclusion_violation OR unique_violation` → returns the existing `{success:false, reason:'slot_taken'}` instead of a raw 23P01 (former dashboard 500 / agent rpc_error on the overlapping-but-different-start GiST race). Advisory lock is in the outer block, untouched by the inner rollback. No caller change. |
| `supabase/migrations/074_service_area_gate.sql` | 2026-06-20 M16 P1 "Service-Area gate" (Capability A), PENDING application — additive, no table rewrites: `service_zones.cities text[] NOT NULL DEFAULT '{}'` (town/city half of the Service-Area coverage list; `postal_codes[]` is the ZIP half); `tenants.out_of_area_action text NOT NULL DEFAULT 'callback' CHECK (out_of_area_action IN ('callback','decline_referral','trip_fee'))` (CHECK-enum mirrors the `tone_preset` style) + nullable `tenants.out_of_area_referral_note`; `inquiries.out_of_area boolean NOT NULL DEFAULT false` (durable flag set by the agent's capture_lead tool via a follow-up service-role UPDATE — the hardened `record_call_outcome` RPC from 069 is deliberately untouched, no RPC overload added). NO RLS policy changes (existing tenant-owner + service_role policies cover the new columns). |
| `supabase/migrations/075_travel_buffer.sql` | 2026-06-20 M16 P2 "Owner-adjustable travel buffer", PENDING application — additive, no table rewrite: `tenants.travel_buffer_mins int NOT NULL DEFAULT 30`. The slot calculator's default buffer path (`_get_travel_buffer_mins` / `getTravelBufferMins`) reads this instead of a hardcoded 30, threaded through `calculate_available_slots`/`calculateAvailableSlots` like `slot_duration_mins`, and enforced forward + backward of every booking. DEFAULT 30 = pre-P2 behavior (zero regression); 0 disables buffering. NO RLS change (the existing tenant-owner policy covers it). Apply BEFORE deploying the Next.js app — the JS available-slots/working-hours routes name the column explicitly and will error pre-migration. The Python agent is pre-migration-safe (all tenant reads use `select("*")` + `.get(...,30)`). |
| `supabase/migrations/076_owner_notification_outbox.sql` | 2026-06-21 LK-B2, PENDING application — new service-role-only `owner_notification_failures` table (durable outbox for failed owner missed-job SMS/email alerts; mirrors `stripe_meter_failures`/071). Columns: id, tenant_id FK ON DELETE CASCADE, `notification_key text UNIQUE` (`'{call_id}:{channel}'`), `channel text CHECK (IN 'sms'|'email')`, recipient, `payload jsonb`, failure_reason, attempts (DEFAULT 0), last_attempt_at, created_at + index on created_at. RLS ENABLED + `REVOKE ALL FROM anon, authenticated`, no policies. Written by the Python agent's post-call §7 (best-effort/fail-open on a 3s owner-send timeout/failure), drained by main-repo cron `/api/cron/retry-owner-notifications` (every 5 min, re-send stored payload, delete on success, Sentry@10). At-least-once delivery (no provider dedupe). Apply BEFORE deploying that cron. |
...
- **JSONB filter syntax for crons** (bypasses the helper — filter at the query level instead): `.eq('features_enabled->>invoicing', 'true')`. Note the value is the **string `'true'`**, not the boolean — PostgREST's `->>` operator returns text. Phase 53 `/api/cron/invoice-reminders` and `/api/cron/recurring-invoices` use this pattern to skip invoicing-disabled tenants at the SQL level. (2026-08-20: both crons are UNSCHEDULED from `vercel.json` and no-op gated on the new `INVOICING_ENABLED` master flag — invoicing v1 freeze; the pattern above still applies when re-enabled.)
- **Invoicing v1 master freeze (2026-08-20)**: new `src/lib/invoicing-enabled.js` exports `INVOICING_ENABLED` (`NEXT_PUBLIC_INVOICING_ENABLED === 'true'`, fail-closed — mirrors `integrations-enabled.js`). While off: `PATCH /api/tenant/features` rejects `invoicing: true` with 403 (disabling stays allowed), the `/dashboard/more/features` toggle renders disabled with a "Coming soon" badge, and the two invoicing crons no-op. This is GLOBAL and distinct from the per-tenant `features_enabled.invoicing` flag — it closes the self-serve enable door so the crons could be unscheduled without a silent-breakage trap. Re-enable checklist lives in the flag file (flip flag + re-add both cron schedules to `vercel.json`).
...
Migrations apply sequentially; the highest migration is 076, with 068–076 PENDING manual apply. FK dependencies require this order. Migrations 001–017 and 019 documented in detail below; 018, 020–036 documented in the file map above with key migrations also detailed below; 052/058–076 documented in the file map above (Phase 54+, Phase 59, Phase 61, prod-readiness 2026-06, Phase 65 voice CHECK swap, 068 billing/security hardening, 069–071 2026-06-12 audit: RPC/token lockdown, ai_voice labels, meter-event outbox; 072/073 quick-win wave; 074 M16 P1 service-area columns; 075 M16 P2 travel buffer; 076 LK-B2 owner-notification outbox) with Phase 59 RPCs/enums detailed in their dedicated sections.
...
| `service_zones` | 003 | Geographic zones for travel buffers. 074: `cities text[] NOT NULL DEFAULT '{}'` — town/city half of the Service-Area coverage (alongside the existing `postal_codes[]` ZIP half). | Tenant child |
...
| `inquiries` | 059 | Unbooked calls. status: open/converted/lost. No cron/auto-timeout (D-07a). 074: `out_of_area boolean NOT NULL DEFAULT false` (set by capture_lead via a follow-up service-role UPDATE). Realtime FULL. | Tenant child |
...
| `owner_notification_failures` | 076 | LK-B2 durable outbox for failed owner missed-job SMS/email ALERTS (mirrors `stripe_meter_failures`). Written by the Python agent's post-call pipeline (upsert on `notification_key` = `'{call_id}:{channel}'`, best-effort/fail-open on a 3s send timeout/failure), drained by `/api/cron/retry-owner-notifications` (every 5 min — re-send stored payload, delete on success, Sentry alert at 10 attempts). At-least-once (no provider dedupe). Columns: id, tenant_id FK, notification_key UNIQUE, channel (CHECK sms\|email), recipient, payload jsonb, failure_reason, attempts, last_attempt_at, created_at. | Service role only (RLS on, no policies, client privileges revoked) |
...
- 074: `out_of_area_action` (text NOT NULL DEFAULT 'callback', CHECK IN 'callback'|'decline_referral'|'trip_fee') + `out_of_area_referral_note` (text, nullable) — M16 P1 Service-Area gate; what the AI does when a caller's address falls outside coverage. The CHECK-enum mirrors the `tone_preset` style. No RLS change (existing `tenants` direct-owner policy covers the new columns).
- 075: `travel_buffer_mins` (int NOT NULL DEFAULT 30) — M16 P2 owner-adjustable travel buffer. The slot calculator's default buffer path reads it (threaded through `calculate_available_slots`/`calculateAvailableSlots` like `slot_duration_mins`), enforced forward + backward of every booking. DEFAULT 30 = pre-P2 behavior; 0 disables. No RLS change. Owner-set via `PUT /api/working-hours`.
...
| `out_of_area` | boolean NOT NULL | DEFAULT false (074) — durable out-of-area flag; set by the agent's capture_lead tool via a follow-up service-role UPDATE, NOT by `record_call_outcome` (the 069-hardened RPC is untouched) |
| `created_at`, `updated_at` | timestamptz | NOT NULL |
...
## 2026-06-12 Audit (cont.): Migrations (072 / 073) — quick-win wave 2026-06-20

Two forward-fix-only migrations. **PENDING manual application** (apply in order; additive + behavior-preserving).

### 072_calendar_blocks_provider.sql — M15 calendar_blocks provider attribution

Adds nullable `calendar_blocks.external_event_provider text` (metadata-only `ADD COLUMN IF NOT EXISTS`, no rewrite). Manual time-block mirrors recorded only `external_event_id`, and the PATCH/DELETE cleanup (`src/app/api/calendar-blocks/[id]/route.js`) resolved the provider by `is_primary` — so a tenant who switched their primary calendar between creating and editing/deleting a block had cleanup hit the WRONG provider. Both write sites (`calendar-blocks/route.js` POST, the `[id]` PATCH/DELETE readers) now record/prefer this column with an `is_primary` fallback for legacy NULL rows, mirroring the appointment-cancel reader (`appointments/[id]/route.js`, hardened in WAVE 1). **Deploy the calendar_blocks code only AFTER this column exists** — the writers/readers reference it. Also: `appointments/route.js` manual-booking POST now writes the already-existing `appointments.external_event_provider` (the appointments half of M15; no migration needed there).

### 073_book_appointment_atomic_slot_taken.sql — LOW-26 clean slot_taken on exclusion race

`CREATE OR REPLACE` of `book_appointment_atomic` (17-arg signature unchanged; SECURITY DEFINER + `search_path` pin + existing ACL all preserved by REPLACE) wrapping ONLY the INSERT in `EXCEPTION WHEN exclusion_violation OR unique_violation THEN RETURN {success:false, reason:'slot_taken'}`. The advisory lock is keyed per (tenant_id, exact start_time) and the pre-check only COUNTs, so a concurrent overlapping-but-DIFFERENT-start booking could pass both and lose the race at the `appointments_no_overlap` GiST constraint, raising a raw 23P01 → dashboard 500 / agent rpc_error. The advisory lock is taken in the OUTER block, so the inner subtransaction rollback doesn't release it. Pure behavior improvement (a former 500 becomes the existing slot_taken contract); no caller change needed (`/api/appointments` POST already maps `reason:'slot_taken'` → 409; the voice agent already recalculates on slot_taken).

---

## M16 P1: Migration 074 — Service-Area Gate (Capability A)

**Written 2026-06-20, PENDING manual application** (additive, no table rewrites; apply after 068–073). File-map entry above; the three column additions are detailed here.

`074_service_area_gate.sql` implements the M16 P1 "Service-Area gate" (Capability A):

- **`service_zones.cities text[] NOT NULL DEFAULT '{}'`** — the town/city half of the Service-Area coverage list, alongside the existing `postal_codes[]` ZIP half. Empty-array default leaves existing zones unaffected.
- **`tenants.out_of_area_action text NOT NULL DEFAULT 'callback'`** with `CHECK (out_of_area_action IN ('callback','decline_referral','trip_fee'))` — what the AI does when a caller's address falls outside coverage. The CHECK-enum mirrors the existing `tone_preset` style. Plus **`tenants.out_of_area_referral_note text`** (nullable) — optional free-text referral note for the `decline_referral` action.
- **`inquiries.out_of_area boolean NOT NULL DEFAULT false`** — durable flag marking an out-of-area lead. Set by the agent's `capture_lead` tool via a **follow-up service-role UPDATE**, NOT through `record_call_outcome`. The hardened `record_call_outcome` RPC (069 lockdown) is deliberately untouched; no RPC overload was added.

**No RLS policy changes** — the existing tenant-owner + service_role policies on `service_zones`, `tenants`, and `inquiries` already cover the new columns.

---

## M16 P2: Migration 075 — Owner-Adjustable Travel Buffer

**Written 2026-06-20, PENDING manual application** (additive, no table rewrite; apply after 068–074). File-map entry above.

`075_travel_buffer.sql` adds a single column:

- **`tenants.travel_buffer_mins int NOT NULL DEFAULT 30`** — the minimum drive time the slot calculator leaves between back-to-back jobs. The calculator's default-buffer path (`_get_travel_buffer_mins` in `livekit_agent/src/lib/slot_calculator.py`; `getTravelBufferMins` in `src/lib/scheduling/slot-calculator.js`) reads this **instead of a hardcoded `30`**, threaded through `calculate_available_slots`/`calculateAvailableSlots` via a `travel_buffer_mins`/`travelBufferMins` parameter exactly like `slot_duration_mins`. So it applies automatically at **both** offer-time and book-time, and is now enforced **forward + backward** of every existing booking (P2 also added the forward adjacency case). `DEFAULT 30` reproduces the exact pre-P2 buffer value → zero regression for every existing tenant; `0` disables buffering. Owner-set on the Working Hours page (`PUT /api/working-hours`, validated int 0–240). Mirrors the `slot_duration_mins` shape (migration 003).

**No RLS policy changes** — the existing `tenants` direct-owner policy covers the new column.

**Deployment ordering:** apply 075 BEFORE deploying the Next.js app. The JS `available-slots` + `working-hours` routes name `travel_buffer_mins` directly and will error against a DB that lacks the column. The **Python agent is pre-migration-safe** — every tenant read uses `select("*")` (incl. `book_appointment`, switched to `select("*")` for parity with `agent.py` so the live booking path has no named-column dependency) and resolves the value via `tenant.get("travel_buffer_mins", 30)`, so a missing column simply defaults to 30.
