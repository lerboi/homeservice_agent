...
| `src/supabase_client.py` | Singleton service-role Supabase client. **2026-09-04 P1.10:** built on a keep-alive `httpx.Client` (HTTP/2, 10 s request cap, `keepalive_expiry=None`) via `SyncClientOptions(httpx_client=…)`; `warm_supabase_connection()` is the never-raising prewarm ping. `VOCO_SUPABASE_KEEPALIVE=false` restores plain `create_client()` |
...
| `src/tools/check_day.py` | List open slots for a whole day; registers `slot_token`s. (Former day-listing branch of `check_availability`.) **2026-09-04 P1.2:** pops `deps["_last_offered_token"]` on entry so a stale check_slot offer can never be booked by book_appointment's recovery branch after the caller switched days |
...
| `src/tools/validate_address.py` | Early mid-call address validation (2026-06-10) — wraps `validate_address_bounded`, caches result on `deps["_validated_address"]`, exports `get_cached_validation` reused by book_appointment/capture_lead. Always-on. M16 P1 (2026-06-20): runs the Service-Area gate (`classify_service_area`) on a confirmed address → `deps["_service_area"]`, overrides to `STATE:address_out_of_area` when out of area. **2026-09-04:** P1.3 code-level address-loop cap (`_validate_attempts`; 2nd unconfirmed same street / 3rd in call → `address_noted`) + P1.4 Singapore postal-first via `onemap.lookup_postal` ahead of Google (`VOCO_SG_ONEMAP`); tool description no longer quotes a literal filler |
...
| `src/integrations/onemap.py` | NEW 2026-09-04 (P1.4): Singapore Land Authority OneMap postal-code lookup — `lookup_postal(postal)` → first result dict (`BLK_NO`, `ROAD_NAME`, `BUILDING`, `POSTAL`, lat/lng) or None; 6-digit guard, per-call `httpx.AsyncClient`, 1.5 s, never raises, no auth |
...
| `src/lib/triage/layer2_llm.py` | LLM urgency classification — **OpenAI `gpt-4.1-nano` since 2026-09-04 (P1.1)** on `OPENAI_API_KEY`; was Groq/Llama 4 Scout (retired by Groq 2026-07-17; silent routine/low fallback since) |
...
| `src/messages/en.json`, `src/messages/es.json` | Agent utterances + notification templates. **2026-09-04 P1.9:** greetings cut to one clause each (disclosure kept), ES `agent.*` strings carry diacritics/inverted punctuation, `recovery_error` reworded (no 'take your details' promise) |
| `pyproject.toml`, `Dockerfile`, `livekit.toml`, `sip-*.json` | Build + deploy config |
| `docs/PLAN-2026-09-02-phase0-phase1.md`, `docs/INFRA-REGIONS.md` | 2026-09-02 execution brief for Phase 0+1 of the call-experience plan (verified anchors, pinned test literals, decisions) + P0.3 region record (Supabase Tokyo verified; Railway/LiveKit regions TO-FILL) |
...
11. **Phase 66 cascade construction** (`src/agent.py` ~L510-551):
    ...
    - `llm = openai.LLM(model=LLM_MODEL, parallel_tool_calls=False,
      max_completion_tokens=500, prompt_cache_key=f"voco-agent:{tenant_id}")`
      — `LLM_MODEL = "gpt-4.1-mini"` (non-reasoning, low TTFT, strong tool
      calling). **2026-09-01 prompt caching:** `prompt_cache_key` (from
      `_prompt_cache_key(tenant_id)`; omitted when there is no tenant row)
      routes a tenant's requests to the machine holding its cached prefix
      (routing hint only — the hit still needs the byte-identical prefix
      that §4's cache-aware layout guarantees). An
      `@llm.on("metrics_collected")` handler — the LLM-object event; the
      AgentSession one is deprecated in 1.5.7 — logs
      `[agent] llm_metrics call= ttft= prompt_tokens= cached_tokens=
      completion_tokens= cancelled=` per request, the only place cache hits
      are visible. Right after `session.start()` the entrypoint fires
      `_warm_prompt_cache(llm, agent.chat_ctx.copy(), agent.tools,
      timeout_s=LLM_CACHE_WARM_TIMEOUT_S)` via `create_background_task`
      (`VOCO_LLM_CACHE_WARM`, default on): one completion with the session's
      exact instructions + tool schemas (`agent.chat_ctx` already holds the
      system instructions message; `agent.tools` is the list the activity
      sends), first chunk read then stream closed, `max_retry=0`, never
      raises — so OpenAI writes the ~9k-token prefix while the greeting
      plays and turn 1 reads it from cache. Why: OpenAI auto-caches
      ≥1024-token prefixes, so turns 2..n already hit, but gpt-4.1-mini has
      only in_memory retention (~5-10 min; no 24h tier) and most tenants
      don't get back-to-back calls, so turn 1 was a cold prefill. Expect
      `cached_tokens≈0` on the warm line and ≈prompt size on turn 1 — a 0
      on turn 1 means the warm didn't land before the caller spoke;
    ...
    - `tts = _build_tts(voice_id, eleven_key_ok=ctx.proc.userdata.get("eleven_key_ok"))`
      (2026-09-04; was inline) — `elevenlabs.TTS(model=ELEVENLABS_TTS_MODEL, voice_id=voice_id)` —
    ...
    - `vad` — Silero, defaults for barge-in. Do NOT port the
      ...
      **2026-08-19:** loaded ONCE per worker process in `prewarm(proc)`
      ...
      as the logged fallback when prewarm didn't run. **2026-09-04 P1.10:**
      `prewarm()` also calls `warm_supabase_connection()` (one
      `tenants.select("id").limit(1)`) so every idle job process already
      holds an open keep-alive TLS session to Supabase (Tokyo) before a
      call lands — log line `[agent] prewarm: supabase connection warmed`;
      `VOCO_SUPABASE_PREWARM=false` skips it. **P1.11:**
      `WorkerOptions(num_idle_processes=NUM_IDLE_PROCESSES)` pins the warm
      pool to `VOCO_NUM_IDLE_PROCESSES` (default 2; SDK default was
      `min(ceil(cpu), 4)`) — watch Railway logs for "no warmed process
      available for job" to raise it.
...
14. Greeting (Phase 66): **deterministic** via
    ...
    `onboarding_complete`, else `agent.greeting_default`. **2026-09-04
    P1.9:** cut from 22 words to one clause — EN "Thanks for calling
    {business_name}. Calls are recorded — how can I help?" / "Hi, thanks for
    calling. Calls are recorded — how can I help?" (ES equivalents with
    diacritics) — because the whole greeting plays with caller input muted;
    the recording clause stays (US two-party states, SG PDPA). No test pins
    the wording beyond `tests/test_messages_greeting.py` (≤14 words,
    "recorded" present, `recording_disclosure` key retained). No LLM turn consumed,
...
14b. **LK-B1 session error recovery (2026-06-21):** `@session.on("error")` still
    ...
    speaks `agent.recovery_error` (since 2026-09-04 P1.9: "Sorry — the line's
    breaking up on my end. Someone from the team will call you right back at
    this number." — the old text promised "let me take your details", which
    this path never does: it speaks the line and hangs up) then drives the SAME
...
customer_context=None, caller_history=None)`.

**Cache-aware layout (2026-09-01):** every tenant-stable section comes
first; the two per-caller blocks (caller history, customer context) are
appended AFTER Transfer, directly ahead of the FINAL recap; `agent.py`
appends `"Today is …"` after the function returns. So for a given tenant the
prompt prefix is byte-identical across calls and OpenAI prompt caching
serves it (see §1 step 11 for `prompt_cache_key` + the greeting-time warm
request). Nothing time-dependent is rendered inside `build_system_prompt`.
Locked by `tests/test_prompt_cache_prefix.py` — **do not move per-caller
data back above the stable sections.**
...
2. **Voice Behavior** — **conciseness-first since 2026-06-10**: leads with
   ...
   collapse. **2026-09-04 P1.7 (acknowledge + act):** the "After the caller
   answers" paragraph now says act — go straight to the next step or lead
   with at most two words ("Okay." "Sure.") — and NEVER restate what the
   caller just said (only a name, an address, a phone number, or a booking
   time is ever read back), with a WRONG/RIGHT pair ("Got it, your toilet
   needs fixing." vs "Okay — what's the address?") and "never start two
   turns in a row the same way" (76% of agent turns were questions, most
   opened by paraphrasing the caller). A matching one-line brevity recap is item 4
   ...
   licensed). **2026-09-04 P1.6 (rotating filler bank):** rules 7 ("never
   say the same filler twice in one call — rotate, and change the shape")
   and 8 (a second call of the same tool in one loop takes a short bridge on
   the acknowledgement — "Okay, seven six eight — trying that." — instead
   of a fresh filler) were added; the example bank is 3–4 differently
   shaped phrases per tool (Let me… / I'll… / Hang on… / present-participle)
   with the "one moment" tail dropped, and the "second-worst … worst"
   closer is gone. The literal "Let me just check that address real quick."
   (spoken 7× across 4 real calls) no longer appears anywhere — the ADDRESS
   VALIDATION opener says "a fresh one — see TOOL NARRATION" and the five
   tool descriptions point at the bank instead of quoting a phrase
   (GPT-4.1 copies the literal nearest the decision point).
   `tests/test_prompt_tool_narration.py` pins: no example phrase twice,
   ≤2 examples with a "one moment"/"one sec" tail, no `'Let me` literal in
   any tool description. **2026-06-12 (P4):** carries the
...
8. **Customer Context (Phase 55/56)** — **position MOVED 2026-09-01**: this
   block and the Caller History block (`_build_caller_history_section`,
   Phase 62 pre-session inject, `CALLER HISTORY (silent context):`) now
   render after Transfer, just before the FINAL recap (cache-aware layout
   above); the rest of this list keeps its relative order.
...
9. **Info Gathering (Phase 60 D-01..D-08)** — outcome-framed. Three needed
   ...
   - NAME USE DURING THE CALL: capture silently; no name vocative
     ...
     honored. **2026-09-04 P1.7:** the acknowledgment bullet is now "two
     words at most and must not contain the caller's name" (the
     "confirm receipt" framing is gone).
   - SERVICE ADDRESS: single-question opener "What's the address where
     ...
     **2026-09-04 P1.4:** `_build_info_gathering_section(t, postal_label,
     locale, country="US")` — for `country == "SG"` ONLY (country-gated,
     never locale-gated) an extra bullet: "In Singapore the postal code pins
     down the building — if the caller gives it, call validate_address
     right away even if the street name was unclear; then ask only for the
     unit number." Pinned by `tests/test_prompt_info_gathering.py`.
...
11. **Booking Protocol (Phase 60 D-02/D-09/D-10; AVAILABILITY rewritten 2026-06-12 P1)**:
    - SCHEDULING: **rewritten 2026-09-04 P1.8** — move to scheduling once
      you have the issue and a confirmed address (get the name on the way,
      never hold booking for it); pick the tool by what the caller gave: a
      day and a time → check_slot now; a day only → check_day now (don't
      ask "what time?" first — offer what it returns); nothing specific →
      next_available_days now. This removed two contradictions that each
      cost a question turn per booking: "only once you have the caller's
      name" (vs NAME USE's "Booking is never blocked by a missing name")
      and "needs both a day and a time … help them decide the other before
      you check" (vs the AVAILABILITY routing). `tests/test_prompt_booking.py`
      pins the new routing text and the absence of the old sentences.
...
- `unconfirmed` → `STATE:address_unclear missing={hint} | DIRECTIVE:ask ONE
  ...
  **2026-09-04 P1.3 — cap enforced in code:** the tool counts attempts in
  `deps["_validate_attempts"]` (per `_norm(street)` key + `__total__`). The
  2nd `unconfirmed` for the same street (`UNCONFIRMED_RETRY_CAP=2`) OR the
  3rd unconfirmed in the call regardless of street
  (`UNCONFIRMED_TOTAL_CAP=3` — STT drifting "Burr/Kenboro/Canberra Drive"
  must not reset the clock) returns `STATE:address_noted speech={as_given}
  | DIRECTIVE:read it back once in the caller's words and continue with the
  next intake step. Do not ask about the address again this call. Never
  mention validation.` The first attempt is byte-identical to before; the
  prompt bullet now reads "ask for just the piece named in `missing=` as a
  plain question … The tool switches to `STATE:address_noted` when it is
  time to stop — never ask about the same piece twice." Rationale: the
  prose "after one retry" rule lost to last-instruction-wins — the 2026-08-17
  Canberra Drive call re-asked the block number 4× in 66 s and the caller
  hung up. Tests in `tests/test_validate_address_tool.py`.
...
**Singapore postal-first via OneMap (2026-09-04 P1.4):** before the Google
call, when `deps["country"] == "SG"`, the postal code is exactly six digits
after stripping spaces/hyphens (`onemap.is_sg_postal`), and
`VOCO_SG_ONEMAP` is not `false`, the tool awaits
`onemap.lookup_postal(postal)` (`src/integrations/onemap.py`: SLA OneMap
`/api/common/elastic/search?searchVal=…&returnGeom=Y&getAddrDetails=Y`,
no auth, per-call `httpx.AsyncClient`, 1.5 s, never raises, rejects a first
result whose `POSTAL` differs). On a hit `_onemap_result()` builds a dict in
the exact `google_maps._voco_result` shape — `formatted_address = "Block
{BLK_NO} {Road Name}[, {Building}]"` (title-cased; Building omitted when
`"NIL"`), `address_components` = `{street_number: BLK_NO, route: road,
subpremise: unit, locality: "Singapore", postal_code, country: "Singapore",
country_code: "SG"}`, lat/lng from OneMap, `place_id: None` — and the
verdict is `confirmed` when the caller's spoken street contains the block
number or fuzzy-matches the road name (`difflib` ratio ≥ 0.8 over the
alphabetic words, common abbreviations Rd/Ave/Dr/Blk/Jln/Lor expanded), else
`confirmed_with_changes` (the Canberra/Kenboro case → the existing
`address_corrected` readback-once branch). Everything downstream —
`_validated_address` cache, `get_cached_validation` reuse by
book_appointment/capture_lead, the service-area gate (reads
`address_components.postal_code`/`.locality`), the STATE builders — runs
unchanged on that dict. No hit / error / flag off / non-SG / non-6-digit →
today's Google path exactly as before (asserted by the patched Google mock
in `tests/test_validate_address_onemap.py`). No `gmaps_validate_events`
row is written for OneMap hits (that table is Google-specific); the log
line is `[validate_address] onemap hit postal= blk= road= building=
verdict=`. Why: an SG postal code identifies one building and STT
transcribes digits reliably while proper-noun street names are what it
mangles — both 2026-08 hang-ups would have resolved on the first attempt.
...
1. Build transcript — `transcript_text` (string) + `transcript_structured`
   (JSON list). **2026-09-04 P0.2:** each entry is `{role, content,
   timestamp}` — `timestamp` is the ms epoch `agent.py` stamped in
   `on_conversation_item`, so caller-turn-end → agent-turn-start gaps are
   computable per call via SQL (`jsonb_array_elements … with ordinality`,
   query kept in the Phase 0+1 brief). Additive: every main-repo consumer
   reads `role`/`content` only.
...
- **Layer 2 — LLM** (`layer2_llm.py`): only when Layer 1 not confident.
  **OpenAI `gpt-4.1-nano` since 2026-09-04 (P1.1)** — `LAYER2_MODEL`,
  `AsyncOpenAI()` with no base_url/api_key (reads the boot-required
  `OPENAI_API_KEY`), JSON mode, temp 0, `max_tokens=100`; the except is
  split (`asyncio.TimeoutError` → `[triage] layer2 timeout` warning; any
  other → `[triage] layer2 failed: …` error) and both still return the
  `routine/low` fallback dict so the post-call pipeline never raises.
  Why: it WAS Groq + Llama 4 Scout — Groq retired that model 2026-07-17 and
  its last non-reasoning Llama models on 2026-08-16 (its remaining
  gpt-oss models are reasoning models whose thinking eats the 100-token
  budget → empty content → JSON error), and the old blanket except had
  silently classified every layer-2 call as routine/low since mid-July
  (`GROQ_API_KEY` was never in the preflight either). Verify after deploy
  with a "water is flooding my kitchen right now" test call → `emergency`.
  **Connection warm (same day):** every call's post-call triage runs in a
  fresh job process, so the classifier's first request was always cold —
  measured ~2.1 s (TLS handshake + generation) against the 2.5 s cap, vs
  ~0.85 s warm. `agent.py` now fires `layer2_llm.warm_client()` (one
  `models.retrieve`, `WARM_TIMEOUT_S=4`, never raises) as a background task
  right after the prompt-cache warm (`VOCO_TRIAGE_LAYER2_WARM`, default
  on); the client uses the SDK's `DefaultAsyncHttpxClient` with
  `keepalive_expiry=None` so the warmed connection is still open at
  post-call time (httpx's default 5 s expiry would have dropped it).
  `TIMEOUT_S` reads `VOCO_TRIAGE_LAYER2_TIMEOUT_S` (default 2.5). Success
  logs `[triage] layer2 ok urgency= confidence= elapsed_ms=` — the number
  to watch on Railway.
  Tests: `tests/test_triage_layer2.py`. Historical: JSON mode, temp 0, **2.5s timeout**
...
`{role, content, timestamp}` — `timestamp` added 2026-09-04 P0.2).
**2026-09-04 P0.1:** the same handler logs one `[agent] turn_metrics call=
role= eot_delay= transcription_delay= llm_ttft= tts_ttfb=` line per turn,
read from `event.item.metrics` (livekit-agents 1.5.7 `MetricsReport`
TypedDict — user turns carry `end_of_turn_delay` (the endpointing wait) /
`transcription_delay`, assistant turns `llm_node_ttft` / `tts_node_ttfb`;
`.get()` because keys are optional). This is the per-stage latency
attribution the call-experience plan is measured with; the handler is
try/except-wrapped so logging can never break transcript capture.
...
| `ELEVEN_API_KEY` | ElevenLabs eleven_flash_v2_5 TTS (Phase 66). **Required at boot** (presence preflight). **Must be the `sk_…` secret, not the key ID** — 2026-09-04 found the key ID in use (ElevenLabs answers `api_key_id_used_as_api_key`); `prewarm()` now verifies it against `/v1/user` (`VOCO_TTS_KEY_PREFLIGHT`, default on) and builds calls on OpenAI TTS directly when rejected, logging `[agent] ELEVEN_API_KEY rejected by ElevenLabs …` |
...
| `VOCO_LLM_CACHE_WARM`, `VOCO_LLM_CACHE_WARM_TIMEOUT_S` | 2026-09-01 prompt caching: background warm request during the greeting (default **on** / **6**s cap) that writes the call's exact system prompt + tool schemas into OpenAI's prompt cache so turn 1 reads a cached prefix instead of cold-prefilling ~9k tokens. `false` disables without a deploy. Cache hits are visible in the `[agent] llm_metrics … cached_tokens=` log lines (see §1 step 11) |
| `VOCO_MIN_ENDPOINTING_DELAY_S`, `VOCO_MAX_ENDPOINTING_DELAY_S` | **P1.5 (pending, 2026-09-04): set `VOCO_MAX_ENDPOINTING_DELAY_S=1.2` on Railway (1.4 if the agent talks over address reads); change the code default only after a few days without talk-over; never lower the min below 0.4.** 2026-08-25 latency pass: post-speech endpointing waits passed to `AgentSession` (defaults **0.4 / 2.0**; SDK defaults were 0.5 / 3.0 — the max applies whenever the semantic turn detector judges the caller unfinished, so it WAS up to 3s of pre-LLM dead air). Env-overridable for instant tuning/rollback |
| `VOCO_TTS_SPEED`, `VOCO_TTS_STABILITY`, `VOCO_TTS_SIMILARITY` | 2026-08-25 pace fix: explicit `elevenlabs.VoiceSettings` on the TTS (defaults **1.08 / 0.5 / 0.75**) — previously no voice_settings were passed, so pace was the voice recording's default cadence (read sluggish). `VOCO_TTS_SPEED=1.0` restores the old pace; construction fails open to the no-settings form. Same commit re-worded the `professional` tone label ("measured and formal" → "polished, warm, and efficient" — "measured" was a slow-delivery instruction in the prompt's first sentence) and de-paced the identity line ("never rush through" → "be clear on") |
...
| `GROQ_API_KEY` | **Unused since 2026-09 (P1.1)** — layer-2 triage moved to OpenAI `gpt-4.1-nano` on `OPENAI_API_KEY`. Leave the Railway var in place for now; safe to delete later |
| `VOCO_TTS_KEY_PREFLIGHT` | 2026-09-04: per-job-process ElevenLabs key check in `prewarm()`; on an explicit auth rejection the call uses OpenAI TTS directly instead of paying ~5 s of FallbackAdapter retries on its first line. Default **on**; `false` restores the always-ElevenLabs-first construction |
| `VOCO_TRIAGE_LAYER2_WARM`, `VOCO_TRIAGE_LAYER2_TIMEOUT_S` | 2026-09-04 P1.1: background `warm_client()` at call start so the post-call gpt-4.1-nano classification is not a cold TLS handshake (default **on**), and the layer-2 `asyncio.wait_for` cap (default **2.5** s — raise only if Railway shows `[triage] layer2 timeout`) |
| `VOCO_SG_ONEMAP` | 2026-09-04 P1.4: Singapore postal-first address resolution via OneMap ahead of Google for SG tenants with a 6-digit postal. Default **on**; `false` restores the Google-only path |
| `VOCO_SUPABASE_KEEPALIVE`, `VOCO_SUPABASE_TIMEOUT_S`, `VOCO_SUPABASE_PREWARM` | 2026-09-04 P1.10: keep-alive HTTP/2 `httpx.Client` under the Supabase client (default **on**; request cap default **10** s — postgrest's own default is 120 s) + the `prewarm()` ping that opens the Tokyo TLS session per idle job process (default **on**). `false` on either restores the previous behaviour without a deploy |
| `VOCO_NUM_IDLE_PROCESSES` | 2026-09-04 P1.11: `WorkerOptions(num_idle_processes=…)`, default **2** (SDK default was `min(ceil(cpu), 4)`). Raise if Railway logs show "no warmed process available for job" |
...
| Every unclear call triaged `routine / low` | Layer-2 classifier failing silently (was the Groq model retirement, 2026-07 → 2026-09) | Check Railway for `[triage] layer2 failed` / `layer2 timeout` (a healthy call logs `[triage] layer2 ok … elapsed_ms=`); the fallback dict is `reason="timeout or error"`. Since P1.1 it runs on OpenAI `gpt-4.1-nano` — verify `OPENAI_API_KEY`, confirm `VOCO_TRIAGE_LAYER2_WARM` is on, and only then consider raising `VOCO_TRIAGE_LAYER2_TIMEOUT_S` |
| Agent confirms one day aloud but the appointment lands on the previous day | Stale `_last_offered_token` reused by book_appointment's malformed-token recovery | Fixed 2026-09-04 P1.2 — `check_day` clears the key on entry. If it recurs, look for a new tool path that offers times without touching `_last_offered_token` |
| Agent re-asks the same address piece 3+ times | Address loop uncapped | Since P1.3 the tool returns `address_noted` on the 2nd unconfirmed for the same street / 3rd in the call — grep `[validate_address] unconfirmed cap reached`; if absent, the model is calling with a different `street` string each time AND fewer than 3 attempts |
| SG address never resolves though the caller gave a postal code | OneMap not consulted | grep `[validate_address] onemap hit`; confirm `tenants.country == 'SG'`, the postal arrived as 6 digits in the tool args, and `VOCO_SG_ONEMAP` is not `false` |
...
| Admin says "I can't talk" / the test call greets then ends with `disconnection_reason='no_input'` and `transcript_structured` has 0 user turns | The console captured a **silent or wrong microphone** — `getUserMedia` succeeded but the OS-default device produces no audio (a virtual/streaming capture device, a muted mic). The agent greets, hears nothing, runs the no-input net, and ends. The `calls` row exists and the recording has agent audio but no caller audio | Fixed 2026-09-05 (`page.js`): the setup card has a **mic device picker + live level meter + silent-device probe**, and the in-call card shows the live input meter ("your mic → agent"). Tell the admin: pick the right device under "4. Microphone", confirm the green bar moves when they speak. A flat bar = that device is what the agent hears (nothing). Confirm via the room-composite recording (`ffprobe`/RMS: caller channel flat = silent mic, not a pipeline fault) |
...
real slot math, triage (incl. the layer-2 LLM call — OpenAI gpt-4.1-nano since 2026-09-04), transcript, recording,
...
| File | Role |
|---|---|
| `src/app/admin/test-agent/page.js` | Console UI: tenant picker, simulated caller number, voice override, **mic device picker + live input-level meter + silent-device probe** (`checkMic`/`startMeter`/`MicMeter`, Web Audio `AnalyserNode`; getUserMedia honors the selected `deviceId`), mic connect (livekit-client), live captions via `lk.transcription` text streams, in-call input meter ("your mic → agent"), mute/hang-up, 10-min timer, post-call results + MP4 player/download |
