...
| `VOCO_MIN_ENDPOINTING_DELAY_S`, `VOCO_MAX_ENDPOINTING_DELAY_S` | 2026-08-25 latency pass: post-speech endpointing waits passed to `AgentSession` (defaults **0.4 / 2.0**; SDK defaults were 0.5 / 3.0 — the max applies whenever the semantic turn detector judges the caller unfinished, so it WAS up to 3s of pre-LLM dead air). Env-overridable for instant tuning/rollback |
